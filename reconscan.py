...
def httpEnum(ip_address, port):
    ...
    HTTPSCAN = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-methods,http-method-tamper,http-passwd,http-robots.txt -oN /root/scripts/recon_enum/results/exam/%s_http.nmap %s" % (port, ip_address, ip_address)
...
def httpsEnum(ip_address, port):
    ...
    print "INFO: Performing nmap web script scan for " + ip_address + ":" + port    
    HTTPSCANS = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts, http-methods,http-method-tamper,http-passwd,http-robots.txt -oX /root/scripts/recon_enum/results/exam/%s_https.nmap %s" % (port, ip_address, ip_address)
...
def nmapScan(ip_address):
   ...
   serv_dict = {}
   TCPSCAN = "nmap -vv -Pn -A -sC -sS -T 4 -p- -oN '/root/%s.nmap' -oX '/root/%s_nmap_scan_import.xml' %s"  % (ip_address, ip_address, ip_address)
   UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oN '/root/%sU.nmap' -oX '/root/%sU_nmap_scan_import.xml' %s" % (ip_address, ip_address, ip_address)
...
if __name__=='__main__':
   f = open('/root/Desktop/recon_scan/ip5.txt', 'r') # CHANGE THIS!! grab the alive hosts from the discovery scan for enum
   for scanip in f:
       jobs = []
